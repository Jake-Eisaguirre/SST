...
# dataInfo <- rerddap::info('erdMWsstd3day_LonPM180')
# parameter <- 'sst'
# xcoord <- c(-121.1, -116.625)
# ycoord <- c(35.1875, 30.7125)
# tcoord <- c('last', 'last')
# xlen <- 0.01
# ylen <- 0.01
...
# raw_chloro <- griddap('erdMWchla3day_LonPM180',
#   time = c('last','last'),
#   latitude = c(31.75, 35.0),
#   longitude = c(-121.5, -116.5),
#   fmt = "csv")
...
# topo <- rxtracto(topoInfo, parameter = 'altitude', xcoord = xlim, 
#                  ycoord = ylim, xlen = .1, ylen = .1)
# 
...
bath_ras <- rasterFromXYZ(so_cal_bath, crs = "+proj=longlat +ellps=WGS84 +datum=WGS84")


# Pals
sst_pal <- colorNumeric(palette = colorspace::sequential_hcl(25,
...
bath_pal <- colorNumeric(palette =colorspace::sequential_hcl(40,
  h = c(320, 200), c = c(40, 65, 5), l = c(15, 98), power = c(0, 1.3)), domain = so_cal_bath$layer)
...
app <- leaflet(options = leafletOptions(minZoom = 8.5)) %>%
  # addMapPane("baseMap", zIndex = 370) %>% 
  # addMapPane("shape_poly", zIndex = 380) %>% 
  # addMapPane("res", zIndex = 390) %>% 
  # averlayerPane("data", zIndex = 450) %>% 
  ...
  addRasterImage(x = re_samp_sst, colors = sst_pal, opacity = 0.7,
                 group = "Sea Surface Temp") %>%
  addRasterImage(x = re_samp_chl, colors = chl_pal, opacity = 0.7,
                 group = "Chlorophyll") %>%
  addRasterImage(x = bath_ras, colors = bath_pal, opacity = 0.7,
                 group = "Bathymetry") %>% 
