...
three_day_sst <- info('erdMWsstd3day_LonPM180')
three_day_sst

raw_sst <- griddap('erdMWsstd3day_LonPM180',
 time = c('last','last'),
...
three_day_chloro <- info('erdMWchla3day_LonPM180')
three_day_chloro
...
mpa <- read_sf(here("/Users","JaketheBoss", "Documents", "Bren", "SST Code", "SST", "SST", "ds582", "ds582.shp")) %>% 
  mutate(geometry = st_transform(geometry, "+proj=longlat +ellps=WGS84 +datum=WGS84")) %>% 
...
app <- leaflet() %>%
  ...
  addPolygons(data = mpa, group = "MPA Status", color = "red") %>% 
  ...
  addLegend(data = r_sst, pal = sst_pal, title = 'Sea Surface Temp', position = "bottomright", values = ~sst, 
            opacity = 1, group = "Sea Surface Temp") %>% 
  addLegend(data = r_chl, pal = chl_pal, title = 'Chlorophyll', position = "bottomright", 
